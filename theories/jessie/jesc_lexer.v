(** Hand-written lexer for Justin/Jessie, producing a MenhirLib token buffer.
    ...
    The token patterns mirror the "Lexical syntax" section of the upstream
    PEG sources, which this lexer transcribes:
    ...
    Each token below is annotated with the upstream production it mirrors.
...
(* _WS <- [\t\n\r ]*;  (quasi-json.js)
   _WS <- super._WS (EOL_COMMENT / MULTILINE_COMMENT)?  (quasi-justin.js) *)
Definition is_ws c :=
  ascii_eqb c " "%char || ascii_eqb c "009"%char
  || ascii_eqb c "010"%char || ascii_eqb c "013"%char.

(* IDENT <- ~(HIDDEN_PFX / IMPORT_PFX / RESERVED_WORD)
            < [$A-Za-z_] [$A-Za-z0-9_]* > _WSN;  (quasi-justin.js) *)
...
(* STRING <- < """" (~"""" character)* """" > _WS;  (quasi-json.js)
   STRING <- super.STRING / """" < (~"""" character)* > """" _WS;  (quasi-justin.js)
...
Fixpoint lex_string_cpt (fuel : nat) (s : string) : option buffer :=
  ...
  | S fuel' =>
    ...
    | String c s' =>
      ...
      (* Punctuation, mirroring the upstream lexical productions:
         LEFT_BRACE <- ""{"" _WS;  RIGHT_BRACE <- ""}"" _WS;  (quasi-json.js)
         LEFT_BRACKET <- ""["" _WS;  RIGHT_BRACKET <- ""]"" _WS;  (quasi-json.js)
         LEFT_PAREN <- ""("" _WS;  RIGHT_PAREN <- "")"" _WS;  (quasi-justin.js)
         _COMMA <- "","" _WS;  COLON <- "":"" _WS;  MINUS <- ""-"" _WS;  (quasi-json.js)
         DOT <- ""."" _WS;  (quasi-justin.js)
         The remaining tokens (SEMI, BANG, LT, EQUALS, ARROW, PLUSEQ,
         MINUSEQ) are Jessie-specific and have no direct upstream lexical
         production; they are transcribed from the grammar's use sites. *)
      | "/"%char =>
        match s' with
